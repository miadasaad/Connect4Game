    ...
    public partial class Form1 : Form
    {
        ...
        public void ReadInfo(object sender, EventArgs e)
        {
            ...
            else if (Msg.Contains("id") == true)
            {
                ...
                {
                    if (room.ID.ToString() == roomId && room.Player2 == null)
                    {
                        //MessageBox.Show("from if room.ID.ToString() == roomId && room.Player2 == null");
                        room.Player2 = (Client)sender;
                        ((Client)sender).bw.WriteLine($"not a viewer");
                        ((Client)sender).bw.Flush();
                    }
                    else if (room.ID.ToString() == roomId && room.Player2 != null)
                    {
                        //MessageBox.Show("from else room.ID.ToString() == roomId && room.Player2 != null");
                        room.viewers.Add((Client)sender);
                        views.Add((Client)sender);
                        ((Client)sender).bw.WriteLine("viewer");
                        ((Client)sender).bw.Flush();
